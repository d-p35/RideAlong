...
public class PostgresDAO {
    ...
    public ResultSet getUsersByEmail(String email) throws SQLException {
        String query = "SELECT * FROM users WHERE email = '%s'";
        query = String.format(query, email);
        return this.st.executeQuery(query);
    }

    public ResultSet addUser(String name, String email, String password) throws SQLException {
        String query = "INSERT INTO users (email, prefer_name, password, rides, isdriver) VALUES ( '%s' , '%s' , '%s' , 0, false )";
