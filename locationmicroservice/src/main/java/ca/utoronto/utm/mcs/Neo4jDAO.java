...
public class Neo4jDAO {
    ...
    public Result getnNearbyDrivers(double latitude, double longitude, double max){
        String query = "WITH %f AS lat, %f AS lon, %f AS max\n" +
                "MATCH (l:user {is_driver: true}) \n" +
                "WHERE point.distance(point({longitude: lon, latitude: lat}), point({longitude: l.longitude, latitude: l.latitude}))/1000  < max\n" +
                "\n" +
                "RETURN collect(l)";
        query = String.format(query,latitude,longitude,max );
        return this.session.run(query);
    }
    ...
    public Result getshortestRoute(String driverUID, String passengerUID){
        ...
        String query = "MATCH(a:user {uid:'%s', is_driver: true})\n" +

                " MATCH(b:user {uid:'%s'})\n" +
                "\n" +
                "MATCH (from:road{ name:a.street }), (to:road { name: b.street}) , \n" +
                "path = AllshortestPaths((from)-[:ROUTE_TO*]->(to))\n" +
                "RETURN  path  AS shortestPath,\n" +
                "    reduce(travel_time= 0, r in relationships(path) | travel_time+r.travel_time) AS totalDistance\n" +
